...
- **Device client (ncclient)** – Enroll devices with a one-time code, then run as a daemon to pull config and certificates and optionally run or restart Nebula when config changes. Install with `pip install nebula-commander`; see [ncclient](/docs/ncclient/) for details.
...
1. Choose an [installation method](/docs/installation/) (development, Docker, or NixOS).
2. [Configure](/docs/configuration/) the backend (database, JWT, and optionally OIDC and SMTP).
3. Use the [Web UI](/docs/usage/web-ui/) and [ncclient](/docs/ncclient/) as needed.
